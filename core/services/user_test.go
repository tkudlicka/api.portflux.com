...
func TestGetByID_Ok(t *testing.T) {
	...
	userRepositoryMock := mocks.NewUserRepository(t)
	userRepositoryMock.On(testutils.FunctionName(t, ports.UserRepository.GetByID), context.Background(), expectedUser.UserID).Return(&expectedUser, nil).Once()
	...
	// Act
	resp, err := service.GetByID(context.Background(), expectedUser.UserID)
...
func TestUpdate_Ok(t *testing.T) {
	...
	userRepositoryMock := mocks.NewUserRepository(t)
	userRepositoryMock.On(testutils.FunctionName(t, ports.UserRepository.GetByID), context.Background(), req.UserID).Return(&existingUser, nil).Once()
	userRepositoryMock.On(testutils.FunctionName(t, ports.UserRepository.Update), context.Background(), req.UserID, mock.AnythingOfType("entities.User")).Return(nil).Once()
	...
	// Act
	err := service.Update(context.Background(), req.UserID, req)
